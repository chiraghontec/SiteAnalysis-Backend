...
@router.post("/climate", response=ClimateResponseSchema)
def analyze_climate(request, coordinates: CoordinateSchema):
    """
    Analyze climate conditions around given coordinates
    """
    try:
        # Initialize the analysis service
        service = EnvironmentalAnalysisService()
        
        # Perform the site analysis first (reuse existing functionality)
        site_analysis = service.analyze_site(
            latitude=coordinates.latitude,
            longitude=coordinates.longitude,
            radius=coordinates.radius,
            site_name=coordinates.name or f"Climate Analysis - {coordinates.latitude:.4f}, {coordinates.longitude:.4f}"
        )
        
        # Initialize climate service and get climate data
        from .services import ClimateDataService
        climate_service = ClimateDataService()
        
        # Get current weather and climate patterns
        current_weather = climate_service._get_current_weather(
            coordinates.latitude, 
            coordinates.longitude
        )
        
        climate_summary = climate_service._get_climate_summary(
            coordinates.latitude, 
            coordinates.longitude
        )
        
        # Create or update climate data for this analysis
        climate_data, created = ClimateData.objects.get_or_create(
            site_analysis=site_analysis,
            defaults={
                'temperature_data': climate_summary.get('temperature', {}),
                'precipitation_data': climate_summary.get('precipitation', {}),
                'wind_data': climate_summary.get('wind', {}),
                'solar_data': climate_summary.get('solar', {}),
            }
        )
        
        # Create summary with station count (mock for now)
        summary = {
            "climate_zone": climate_summary.get("zone", "temperate"),
            "temperature_range": climate_summary.get("temperature", {}),
            "precipitation_annual": climate_summary.get("precipitation", {}).get("annual_mm", 0),
            "wind_patterns": climate_summary.get("wind", {}),
            "solar_exposure": climate_summary.get("solar", {}),
            "analysis_radius": coordinates.radius,
            "center_coordinates": {
                "latitude": coordinates.latitude,
                "longitude": coordinates.longitude
            }
        ...
        return {
            ...
            "radius": site_analysis.analysis_radius,
            "current_weather": current_weather,
            "stations_count": 1,  # Mock value - could be expanded with real weather stations
            "summary": summary,
            "created_at": site_analysis.created_at.isoformat(),
        }
        
    except Exception as e:
        logger.error(f"Error in analyze_climate: {str(e)}")
        return JsonResponse(
